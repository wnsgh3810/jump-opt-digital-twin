# -*- coding: utf-8 -*-
"""P19 CMA-7 — 무캘리브레이션 배포층: 전역 softsat(a,s) + 지연, α 세션보정 없음. 물리 재적합 — 결합 목적: J = 0.5·CL τ-갭 + 0.3·ModeA(평행사변형 점프, x0 정규화)
...
LO = np.array([0.0, 1.5, 0.0, 0.05, 0.0, 0.0, 0.0, 0.002, 0.05, 0.0005, 0.40, 0.50, 0.55, -0.05, -0.08, 0.0, -0.09, -0.11, 0.6, 8.0])
HI = np.array([2.5, 2.7, 4.0, 2.50, 0.30, 0.15, 0.15, 0.030, 0.70, 0.020, 1.10, 1.50, 1.90, 0.16, 0.05, 0.020, 0.09, 0.02, 1.5, 60.0])
...
def parts_x(v):
    ...
    rows = R.eval_stack(x32, v[1], sp, P.A_PAPER, v[2], v[15], use_alpha=False,
                        q_off_0429=(v[16], v[17]), softsat=(v[18], v[19]))
...
def main():
    ...
    x0 = np.array(list(W2) + [0.919, 38.5])
    ...
    while not es.stop():
        ...
        json.dump(dict(J=float(best[0]), x=[float(v) for v in best[1]], names=NAMES,
                       nev=nev), open(HERE / "p19_cma7.json", "w"), indent=1)
    print(f"CMA7 DONE nev={nev} J={best[0]:.4f} [{(time.time()-t0)/60:.1f}m]", flush=True)
