# -*- coding: utf-8 -*-
"""P19 CMA-2 — 결합 목적: J = 0.5·CL τ-갭 + 0.3·ModeA(평행사변형 점프, x0 정규화)
...
LO = np.array([0.0, 1.5, 0.0, 0.05, 0.0, 0.0, 0.0, 0.002, 0.05, 0.0005, 0.40, 0.55, 0.55, -0.05, -0.08, 0.0])
HI = np.array([2.5, 2.7, 4.0, 2.50, 0.30, 0.15, 0.15, 0.030, 0.70, 0.020, 1.10, 1.45, 1.45, 0.08, 0.05, 0.020])
...
def modeA_429(x32, ref, sp):
    ...
    try:
        ...
        for sub in SUB4:
            ...
            if L is None:
                return 9e3
            scs.append(score(metrics2(d, L, 3.14 * np.pi / 180, -3.0 * np.pi / 180)))
...
def main():
    ...
    import cma
    maxfev = int(sys.argv[1]) if len(sys.argv) > 1 else 700
    X = P.X37
    ...
    json.dump(base, open(HERE / "p19_cma2_base.json", "w"), indent=1)
    ...
    while not es.stop():
        ...
        json.dump(dict(J=float(best[0]), x=[float(v) for v in best[1]], names=NAMES,
                       nev=nev), open(HERE / "p19_cma2.json", "w"), indent=1)
    print(f"CMA2 DONE nev={nev} J={best[0]:.4f} [{(time.time()-t0)/60:.1f}m]", flush=True)
