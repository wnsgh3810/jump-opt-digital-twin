"""P16 통합 재적합 — P14 + springref 자유화 (37-dim). 원본: p14_fit.py — 이중 심판 동시 최소화, x = 32 모델 + 4 a_hat 계수.
...
def winit():
    J.winit()
    import p16a_spring as PS2
    J.build_model = lambda x32: PS2.build_with_ref(x32, _REF[0])


def eval37(x37):
    x37 = list(x37)
    _REF[0] = float(x37[36])
    return J.eval36(x37[:36])
...
def main():
    ...
    NAMES = h["names"] + ["A1", "A2", "A3", "A4", "springref"]
    p14c = json.load(open(HERE.parent / "p14_ahat/fourbar_p14_candidate.json"))
    x0 = np.concatenate([np.array(p14c["x"]), [2.4]])
    LOb = np.concatenate([FR.LOb.copy(), P13.LO6.copy(),
                          [1.00, 0.0, 0.0, 0.0], [0.0]])
    HIb = np.concatenate([FR.HIb.copy(), P13.HI6.copy(),
                          [1.35, 1.3e-3, 0.45, 0.10], [2.6]])
        ...
        for x, r in zip(xs, rs):
            ...
            if o < best["obj"]:
                ...
                print(f"BEST nev={nev} obj={o:.4f} hoA={hoA:.3f} hoC={hoC:.3f} "
                      f"A=[{A[0]:.3f},{A[1]:.2e},{A[2]:.3f},{A[3]:.3f}] ref={ref:.2f} "
    ...
    if sel:
        ...
        json.dump(dict(CANDIDATE="P16 — P14 + springref free (2026-07-09)", springref=ref,
                       sens_delay=-0.0015, obj=sel["obj"], hoA=sel["hoA"], hoC=sel["hoC"],
                       A_HAT=[float(v) for v in A], names=NAMES, x=sel["x"]),
                  open(HERE / "fourbar_p16_candidate.json", "w"), indent=1)
