...
B = [
    callout("🏋️", rt("질문: ", bold=True),
            ...
            rt("답: 폐루프 검증 4/4 통과 (q2 0.02~0.07 rad, GRF가 페이로드에 정확 비례 31→56→80N), "
               "반사실 실험으로 no-CVT 7.5kg 기립 실패를 트윈이 재현, 같은 로드에서 CVT가 무릎 모터 토크 −18~23%, "
               "순간 천장 기준 페이로드 한계 ~5-6kg → ~8-9kg (+60%).", bold=True)),
    ...
    table([
        ...
        ["no_cvt/load_5", "기립 실패 — xlsx 미수출 (영상만)", "30mm", "반사실 대상"],
        ["no_cvt/load_7.5", "기립 실패 — xlsx 미수출", "30mm", "반사실 대상"],
    ]),
    ...
    table([
        ...
        ["no_cvt + 5.0kg", "30.1cm ✓", "15.9", "0%", "성공 (트윈) — 실기는 실패, 아래 주의"],
        ["no_cvt + 7.5kg", "−2.1cm ✗", "발산", "85%", "기립 실패 — 실기 실패 재현"],
        ["cvt + 7.5kg", "31.5cm ✓", "17.5", "0%", "성공 (실기 미실험 조합)"],
        ["cvt + 10kg", "33.2cm (한계)", "26.0", "49%", "한계선"],
    ]),
    callout("⚠️", rt("정직한 주의 — no_cvt 5kg: ", bold=True),
            rt("실기는 실패했지만 트윈(순간 천장 기준)은 성공(15.9 < 18.9Nm). s2s는 점프(0.2s 버스트)와 달리 지속 부하이므로, "
               "실기 실패의 유력 원인은 순간 천장이 아니라 AK80-9 V2 정격(연속) 9Nm의 열 제한 — no_cvt 5kg는 정격의 1.8배를 ~1초 지속해야 함. "
               "트윈에는 열 모델이 없어 이 축은 재현 불가 (모터 온도 로그가 있으면 검증 가능).")),
    ...
    quote(rt("s2s 자세 범위(무릎 60~130°)에서 전달비 r ≈ 0.8 → 무릎 모터 토크 ≈ ×0.8 — 측정·시뮬 절감치(−18~23%)와 정합. "
             "순간 천장 기준 페이로드 한계: no-CVT ~5-6kg → CVT ~8-9kg (+~3kg, +60%). "
             "지속(정격 9Nm) 기준으로도 같은 비율로 한계 확장 — 실기에서 no_cvt 5kg 실패 / cvt 5kg 성공의 차이가 정확히 이 마진.", bold=True)),

    h1("5. 그림"),
]
imgs = [
    (DST / "torque_margin.png", "무릎 모터 토크 수요 vs 공급 한계 — 로드별, CVT/no-CVT, 실측+반사실. 빨강 점선=순간 천장 18.9Nm, 보라=정격(연속) 9Nm"),
    ...
    (DST / "counterfactual/no_cvt_CF_load7.5__CLcap.png", "반사실: no_cvt + 7.5kg + 천장 — 무릎이 천장에 걸려 기립 실패 (실기 실패 재현)"),
