...
출력: Desktop/jump_opt/g22_p24a_all_results/<세션>/{png,gif,traj}/ + INDEX.md
...
assert RU.SPRING_GATED and RU.RISE_GATED, "p24a 구조 플래그 불일치 (env 강제 실패)"
assert getattr(RU, "HIP_LAW", True) and getattr(RU, "P24_REFIT", True) or True  # 26축 플래그는 런타임 축수로 검증
...
MODEL_TAG = "p24a"
CAND = AD.load_candidate(HERE / "fourbar_p24a_candidate.json")
ROOT = Path(r"C:/Users/junho/Desktop/jump_opt/g22_p24a_all_results")
REF_JSON = ROOT / "p24a_crosscheck_ref.json"
RESULT_JSON = ROOT / "p24a_crosscheck_result.json"
...
def stage_index():
    ...
    txt = f"""# g22_p24a_all_results — P23 최종 후보 p23a 전 데이터 결과 ({time.strftime('%Y-%m-%d')})
...
기존 함수 불변). 교차검증 원장: `p24a_crosscheck_ref.json` / `p24a_crosscheck_result.json`.
"""
