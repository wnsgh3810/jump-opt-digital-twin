...
def v23_p19_law():
    ...
    v = np.concatenate([x19_20(), [0.0, 0.0]])
...
def v23_p22b_law():
    ...
    v = np.concatenate([RN.x22b_vec(), [0.0, 0.0]])
    v[15], v[16], v[19] = LAW_A0, LAW_V00, LAW_B0
    return np.clip(v, LO23 + 1e-9, HI23 - 1e-9)
...
def build_flip23(x32, ref, sp, d_dq):
    model, _ = C._W["P"].build_flip(x32, ref, sp)
    return _patch_ddq(model, d_dq)


def build_cvt23(x32, ref, sp, l_i, d_dq):
    model, _ = C._W["P"].build_cvt(x32, ref, sp, l_i)
    return _patch_ddq(model, d_dq)


def build_weld23(x32, ref, sp, d_dq):
    model, _ = RN.build_flip_welded(x32, ref, sp)
    return _patch_ddq(model, d_dq)
...
def cl_run23(model, is_cvt, l_i, d, gains, dqdes_on, ffk, A, tm, alphas, law,
             c_cvt=0.0, o1=0.0, o2=0.0, ff_hip=False):
...
def cl_metrics23(v, x32, sp, law, c_cvt, d_dq, model_f=None):
...
def windows23(model, x32, dss, law, W_override=None):
...
def win429_06_23(x32, sp, ref, law, c_cvt, d_dq):
        ...
        for i0, r_, gp in pre["starts"]:
            ...
            for k in range(nst):
                ...
                if qg is not None:                              # ★ C_CVT
                    ...
                    data.qfrc_applied[dof_knee] = -c_cvt * abs(s2m) * amp * float(np.tanh(vk / 1.0))
                try:
...
def score_0604_23(x32, sp, ref, law, c_cvt, d_dq):
    """p20_cma2.score_0604 세대 교체 — lam→supp_vec + D_DQ + C_CVT. pre604 = winit 사전계산."""
    P, mj = C._W["P"], C._W["mj"]
    MS = C._W["P12"]._G["MS"]
    per = []
...
def a_full23(model, is_cvt, l_i, d, law, o1, o2, c_cvt=0.0):
    ...
    for k in range(N):
        ...
        if qg is not None:                                      # ★ C_CVT (CVT 한정)
            ...
            amp = max(1.0 / max(abs(rr), 0.2) - 1.0, 0.0)
            vk = float(md.qvel[dof_knee])
            md.qfrc_applied[dof_knee] = -c_cvt * abs(s2) * amp * float(np.tanh(vk / 1.0))
        try:
            mj.mj_step(model, md)
        except Exception:
            return None
...
def oldq_h23(v, x32, sp, law, c_cvt, d_dq, model_f=None):
...
def cl_ff23(x32, sp, ref, tm, law, d_dq, ff_hip, model_f=None):
...
def oldq_ff23(x32, sp, ref, law, d_dq, model_f=None):
    """p23_runners.oldq_ff 세대 교체 — a_full23 (o1=o2=0)."""
    if model_f is None:
        model_f = build_flip23(x32, ref, sp, d_dq)
    rows = []
    for ds, sub, d, gains, dqon, ffk, m, is_cvt, l_i in RN.ff_trials():
        res = a_full23(model_f, False, l_i, d, law, 0.0, 0.0, c_cvt=0.0)
        hr = float(d.get("h_real", float("nan")))
        if res is None:
