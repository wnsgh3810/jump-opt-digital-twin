# -*- coding: utf-8 -*-
"""P19 CMA-6 — 링키지 관성 N6 (s_rc,s_ic,s_rp,s_ip,d_cpin,d_kneep) 해방 + 전 22dim — 결합 목적: J = 0.5·CL τ-갭 + 0.3·ModeA(평행사변형 점프, x0 정규화)
...
NAMES = ["stiff", "ref", "pre30", "fv_hip", "fc_hip", "fv_knee", "fc_knee",
         ...
         "o1_429", "o2_429", "s_rc", "s_ic", "s_rp", "s_ip", "d_cpin", "d_kneep"]
...
LO = np.array([0.0, 1.5, 0.0, 0.05, 0.0, 0.0, 0.0, 0.002, 0.05, 0.0005, 0.40, 0.50, 0.55, -0.05, -0.08, 0.0, -0.09, -0.11, 0.3, 0.3, 0.5, 0.3, 0.0, 0.0002])
HI = np.array([2.5, 2.7, 4.0, 2.50, 0.30, 0.15, 0.15, 0.030, 0.70, 0.020, 1.10, 1.50, 1.90, 0.16, 0.05, 0.020, 0.09, 0.02, 2.0, 3.0, 1.5, 3.0, 0.05, 0.05])
...
def main():
    ...
    x0 = np.array(list(W2) + [float(P.X37[26 + i]) for i in range(6)])
    ...
    while not es.stop():
        ...
        json.dump(dict(J=float(best[0]), x=[float(v) for v in best[1]], names=NAMES,
                       nev=nev), open(HERE / "p19_cma6.json", "w"), indent=1)
    print(f"CMA6 DONE nev={nev} J={best[0]:.4f} [{(time.time()-t0)/60:.1f}m]", flush=True)
