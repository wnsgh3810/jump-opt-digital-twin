"""cl_fit3 — 게인-적합 v3: q1·dq2 우선 (사용자 07-09).

fit2와 동일 골격 (구간 가중 초0.5/푸시2/비행1, 채널 정규화=라벨 런 1.0, τ채널 유지로
널-드리프트 봉쇄)에 채널 가중만 추가: q1=3, dq2=3, 나머지(q2,dq1,tau1,tau2)=1.
...
def fit_one(args):
    ...
    np.savez(TRAJD / f"{ds}__{sub}__fit3.npz",
    ...
    return dict(ds=ds, sub=sub, obj=float(sf), gains_label=[float(v) for v in lg],
                gains_fit3=[float(v) for v in gf],
