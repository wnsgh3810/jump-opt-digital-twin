...
STD_FLOOR_UNTIL = 10_000_000     # [env steps] 이후 플로어 해제
HID = {"nc": 64, "wc": 64, "wc2": 128}   # wc2: 용량 확대
...
def load_env_module(campaign):
    ...
    EVm.NOISE_Q = NOISE_Q_LONG[campaign]
...
def worker_main(conn, campaign, seeds):
    ...
    envs = [EVm.JumpEnv(seed=s, reset_noise=True) for s in seeds]
...
def collect_teacher(EVm, campaign, npz_name, li):
...
def main(campaign):
    ...
    for npz_name, li in teachers:
        X, A, meta = collect_teacher(EVm, campaign, npz_name, li)
    ...
    log = dict(golden=golden, updates=[], evals=[], hyper=dict(
        ...
        total_steps=TOTAL_STEPS, gamma=GAMMA, lam=LAM, clip=CLIP_EPS,
        ...
        plateau=dict(after=PLATEAU_AFTER, patience=PLATEAU_PATIENCE,
                     delta=PLATEAU_DELTA),
        std_floor=(dict(floor=STD_FLOOR, until=STD_FLOOR_UNTIL)
                   if campaign == "wc2" else None),
        explore=dict(bc=dict(teachers=t_meta, iters=BC_ITERS, lr=BC_LR,
                             mse=[mse0, mse1], det_h_after_bc=bc_h),
                     noise_q=NOISE_Q_LONG[campaign], noise_dq=NOISE_DQ_LONG,
                     log_std0=LOG_STD0,
                     crouch=("teacher q0 " + str(EVm.G["CROUCH"]) if campaign == "nc"
                             else "teacher q0(l_i) 선형보간 "
    ...
    for up in range(1, n_updates + 1):
        ...
        nb = b_obs.shape[0]
        idx = np.arange(nb)
        warm = up <= VF_WARM       # value-only 워밍업 (BC 정책 보호)
        for _ in range(EPOCHS):
            ...
            for s in range(0, nb, MB):
                ...
                opt.step()
                if campaign == "wc2" and steps_done < STD_FLOOR_UNTIL:
                    with torch.no_grad():      # 엔트로피 플로어 (wc2 처방 ②)
...
if __name__ == "__main__":
    ...
    assert camp in ("nc", "wc", "wc2"), "usage: python t0_train_long.py nc|wc|wc2 [steps]"
    if len(sys.argv) > 2:
