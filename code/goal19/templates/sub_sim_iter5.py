"""Iter5: 31 sub-experiment simulator with 5 mass per-component params.

Axes (NEW vs iter4):
  - M_thigh_scale: scales M1_CAD (thigh primary mass)
  - M_calf_scale : scales M2_CAD (calf primary mass)
  - M_p_scale    : scales M_P_CAD (pulley) — affects thigh composite + COM
  - M_c_scale    : scales M_C_CAD (chain/connector) — affects calf composite + COM
  - M_foot_extra : absolute kg [0, 0.5] (foot rim/cylinder extra mass)

Locks (from iter4 best):
  - motor_tm  = 0.032s
  - m_base_scale = 1.0358  → M_base = M_BASE_CAD * 1.0358
  - solref_tc = 0.007085
  - imp0      = 0.2526
  - friction  = 0
...
# ── Composite inertia with 5D mass split ──────────────────────────────────────
def ci_5d(m_thigh_scale=1.0, m_calf_scale=1.0,
          m_p_scale=1.0, m_c_scale=1.0, m_foot_extra=0.018461):
    """Composite mass + COM + I for thigh and calf using 5D mass split.

    NOTE — Inertia I1/I2 are physically tied to their masses (parallel axis
    base). Scale inertia "primary" component by mass scale to be consistent.
    """
    M1 = M1_CAD * m_thigh_scale
    Mp = M_P_CAD * m_p_scale
    M2 = M2_CAD * m_calf_scale
    Mc = M_C_CAD * m_c_scale
    Mf = float(m_foot_extra)

    I1 = I1_VAL * m_thigh_scale
    Ip = IP_VAL * m_p_scale
    I2 = I2_VAL * m_calf_scale
    Ic = IC_VAL * m_c_scale

    # Thigh composite (M1 at R1, M_P at RP, both around hip)
    Mt = M1 + Mp
    ...
    # Calf composite (M2 at R2, M_C at RC, M_FOOT at L2_VAL)
    Mc2 = M2 + Mc + Mf
...
# ── Jump XML ──────────────────────────────────────────────────────────────────
def build_xml_jump_5d(mts, mcs, mps, mcps, mfx):
    Mt, ctz, It, Mc2, ccz, Ic2 = ci_5d(mts, mcs, mps, mcps, mfx)
    sr = _solref_str(); si = _solimp_str()
    M_base = _base_mass()
    return f"""<mujoco model="iter5_jump_5d">
...
<worldbody>
  ...
  <body name="base" pos="0 0 0" childclass="leg">
    ...
    <body name="thigh" pos="0 0 -0.025">
      <joint name="hip" type="hinge" armature="0" damping="{FV_HIP:.8f}" frictionloss="{FC_HIP:.8f}" stiffness="0" springref="0"/>
      ...
      <body name="calf" pos="0 0 -{L1_VAL}">
        <joint name="knee" type="hinge" armature="0" damping="{FV_KNEE:.8f}" frictionloss="{FC_KNEE:.8f}" stiffness="0" springref="0"/>
...
def build_xml_sit2stand_air_5d(mts, mcs, mps, mcps, mfx):
    Mt, ctz, It, Mc2, ccz, Ic2 = ci_5d(mts, mcs, mps, mcps, mfx)
    sr = _solref_str(); si = _solimp_str()
    M_base = _base_mass()
    return f"""<mujoco model="iter5_s2s_air">
...
<worldbody>
  ...
  <body name="base" pos="0 0 1.5" childclass="leg">
    ...
    <geom type="box" size="0.06 0.03 0.025" contype="0" conaffinity="0"/>
    <body name="thigh" pos="0 0 -0.025">
      <joint name="hip" type="hinge" armature="0" damping="{FV_HIP:.8f}" frictionloss="{FC_HIP:.8f}"/>
      <inertial pos="0 0 {ctz:.5f}" mass="{Mt:.5f}" diaginertia="{It:.6f} {It:.6f} 0.0002"/>
      <geom type="capsule" size="0.02" fromto="0 0 0 0 0 -{L1_VAL}" contype="1" conaffinity="1"/>
      <body name="calf" pos="0 0 -{L1_VAL}">
        <joint name="knee" type="hinge" armature="0" damping="{FV_KNEE:.8f}" frictionloss="{FC_KNEE:.8f}"/>
        <inertial pos="0 0 {ccz:.5f}" mass="{Mc2:.5f}" diaginertia="{Ic2:.6f} {Ic2:.6f} 0.00005"/>
        <geom type="capsule" size="0.015" fromto="0 0 0 0 0 -{L2_VAL}" contype="1" conaffinity="1"/>
        <geom name="foot" class="foot" type="cylinder" size="{FOOT_RADIUS:.4f} {FOOT_HALF_LEN:.4f}" pos="0 0 -{L2_VAL}" euler="90 0 0"/>
      </body>
...
def build_xml_sit2stand_gnd_5d(mts, mcs, mps, mcps, mfx):
    Mt, ctz, It, Mc2, ccz, Ic2 = ci_5d(mts, mcs, mps, mcps, mfx)
    sr = _solref_str(); si = _solimp_str()
    M_base = _base_mass()
    return f"""<mujoco model="iter5_s2s_gnd">
<option cone="{CONE}" impratio="{IMPRATIO}" gravity="0 0 -9.81" timestep="{DT}" integrator="implicitfast"/>
<default><default class="leg">
  <geom friction="1.0 0.02 0.01" margin="0.001" condim="6"/>
  <joint axis="0 1 0"/>
...
def run_one_sub(ds, sub, q1_off, q2_off,
                m_thigh_scale, m_calf_scale, m_p_scale, m_c_scale, m_foot_extra,
    ...
    if ds in JUMP_LOADERS:
        ...
        td = loader(sub)
        xml = build_xml_jump_5d(m_thigh_scale, m_calf_scale, m_p_scale, m_c_scale, m_foot_extra)
    ...
    if full_id in SIT2STAND_NPZ:
        ...
        if cyc_list is None or len(cyc_list) == 0:
            return None, None
        if is_gnd:
            xml = build_xml_sit2stand_gnd_5d(m_thigh_scale, m_calf_scale, m_p_scale, m_c_scale, m_foot_extra)
        else:
            xml = build_xml_sit2stand_air_5d(m_thigh_scale, m_calf_scale, m_p_scale, m_c_scale, m_foot_extra)
        model = mujoco.MjModel.from_xml_string(xml)
...
if __name__ == "__main__":
    ...
    # Smoke test: baseline (all scales = 1, M_FOOT = 0.018461 default)
    o = [x for x in offsets if x['ds'] == 'jump_0424' and x['sub'] == '60_0.75_60_2'][0]
    t0 = time.time()
    s, m = run_one_sub(o['ds'], o['sub'], o['q1_off'], o['q2_off'],
                       1.0, 1.0, 1.0, 1.0, 0.018461)
    print(f"jump smoke (mts=1, mcs=1, mps=1, mcps=1, mfx=0.018): score={s:.2f} ({time.time()-t0:.2f}s)")
    if m is not None:
        print(f"  rmse_q1={m['rmse_q1']:.3f} pen_mm={m['pen_max_mm']:.3f} foot_z_min={m['foot_z_min_m']:.4f}")
