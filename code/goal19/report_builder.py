...
<h3>per-date 엔코더 offset</h3>
<p class="body">0602(6월, 최상 캘리브레이션)를 기준으로 세션별 q1/q2 offset을 두면 −16.3% —
사용자의 "3, 4월 데이터는 캘리브레이션도 잘못했다"는 증언과 정확히 일치하는 물리적 보정입니다
(per-trial이 아닌 per-date 상수, 미래 세션에서도 세션 캘리브레이션으로 대응).</p>
...
MIT의 결론(arXiv 2404.15096)과 일치. ※주의: 단일 seed IPOPT라 k=40000 행처럼 국소해 비단조 존재.
실무 지침: <b>NLP 접촉을 식별된 트윈 접촉(solref 0.006/imp0 0.371 등가강성)에 맞추고 마찰 포함</b>.</p>
</div></section>
