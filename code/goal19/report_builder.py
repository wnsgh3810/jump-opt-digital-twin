...
<b>0324 무릎 방향만 진짜 미모델 계통 차이를 흡수</b> 중 — 엔코더가 아닌 3월 하드웨어 상태
(예: CVT l_i 세팅이 30mm와 달랐다면 1:1 평행사변형이 깨져 자세 의존 각도차 발생)가 유력 후보로 조사 대상.
±2° 제약 재적합(CMA 220 evals)은 <b>개선 0.0%로 즉시 수렴, 물리 파라미터 전부 불변</b> —
offset이 하던 일을 어떤 물리 파라미터도 대체 못함 = 순수 "각도 기준선" 성격의 세션 계통차 확정.
핵심: <b>물리 트윈은 두 버전에서 동일하고, held-out h와 배포/NLP는 offset을 사용하지 않으므로
전이 목적에는 영향 없음</b>. offset은 과거 세션 채점용 nuisance 파라미터로 재분류(±2° 제약 채택),
0324의 잔여 무릎 편향(~6° 상당)은 원인 미상 3월 계통차로 정직하게 기록.</p>
