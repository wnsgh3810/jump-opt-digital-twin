...
# ── 현행 스택 H3_260812 의 11 개 값 (1 회차 산물, `_GHB_sweep.json` res.canon_cap.x) ──
#   순서 = COMMON 9 개 + EXTRA["canon_cap"] 2 개. **스윕의 출발점이자 이겨야 할 상대다.**
...
# 그 전 스택 H2 의 11 개 값 = **점수의 기준선**(아래 BASE_ENV 와 같은 지점). 이걸 넣으면
# 1.0000 근처가 나와야 한다 — 판이 제대로 서 있는지 매 실행 첫머리에 확인하는 용도다.
H2 = [0.2469, 0.2383, 0.150, 0.312, 3.28, 0.0, 150.0, 0.002, 0.0025, 3.8, 2.6]
...
COMMON = [
    ("무릎 건마찰",      0.15, 0.60, 0.2880),   # 매달림 실측 0.46 (H2 시작값 0.2469)
