...
def build(out_dir=None):
    ...
    A("\n열 설명\n")
    A("- **푸시 극값**: 푸시 구간 **안에서** 슬립이 간 최소~최대. 끝점차와 크게 다르면 "
    ...
    for r in rows:
        if r["sess"] != prev:
            prev = r["sess"]
            A(f"\n## {r['sess']}  ({r['fps']:.0f}fps)\n\n")
            A("| trial | 자<br>mm/px | 지름<br>px | 하강<br>전반 | 하강<br>후반 | **하강<br>합** | 바닥<br>유지 "
              "| **푸시** | 푸시<br>극값 | 전체<br>슬립 | 전체<br>구름 | 전체<br>Δx | sens | 절단 | QC |\n")
            A("|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|\n")
        ex = (f"{r['pmin']:+.1f}~{r['pmax']:+.1f}" if np.isfinite(r["pmin"]) else "—")
        A(f"| {r['trial']} | {r['sc']:.4f} | {r['dia']:.1f} | {r['d1']:+.1f} | {r['d2']:+.1f} "
          f"| **{r['de']:+.1f}** | {r['fl']:+.1f} | **{r['pu']:+.1f}** | {ex} | {r['tot']:+.1f} "
          f"| {r['rl']:+.1f} | {r['dx']:+.1f} | {r['sens']:.1f} | {r['why']} | {r['qc']} |\n")
