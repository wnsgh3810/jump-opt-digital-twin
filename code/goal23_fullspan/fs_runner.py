...
def _tmap_init(P=None, A=None):
    ...
    if md_ in ("canon", "canon_cap", "canon_env", "canon_fric", "canon_capv", "canon_fv", "canon_capS", "canon_mix"):
        ...
        def _canon(raw):
            # 명령 토크(raw)를 정본 곡선의 역함수로 풀어 축 토크로 바꾼다.
            #   d1·t + d2·t·|t| + d3·t³ = raw  를 t 에 대해 뉴턴 방법으로 푼다.
            # ★ 08-12 밤: 반복을 40 → 10 으로 줄였다 (속도 4 배).
            #   이 함수가 스윕 계산 시간의 34% 를 먹고 있었다 — 매 계산 스텝마다 두 관절에
            #   대해 호출되고, 한 번 부를 때마다 40 바퀴를 돈다.
            #   줄여도 되는 근거: 명령 −45~45 N·m 를 4001 점으로 훑어 40 회 결과와 대조했더니
            #     4 회 2.1e-06 · 6 회 7.1e-15 · **10 회 정확히 0** · 20 회 3.6e-15.
            #   즉 6 바퀴면 소수점이 더 안 움직이고, 10 바퀴는 부동소수점 자리까지 동일하다.
            #   나머지 30 바퀴는 같은 값을 30 번 다시 계산하는 순수 낭비였다.
            t = raw / d1
            for _ in range(10):
