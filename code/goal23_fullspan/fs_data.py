...
def vdes_applied(sess) -> bool:
    """이 세션에서 **목표 속도가 실제로 인가되었나** — PD 제어 흉내의 식이 달라진다.

    사용자 확정 08-13: **0424 이전 세션은 전부 목표 속도를 0 으로 잘못 넣고 제어했다.**
    즉 26.03.19 / 26.03.24 / 26.04.21 은 실효 PD 가
    `게인x(목표각-실제각) - 감쇠x(실제 속도)` 이고 목표 속도 항이 없다. 0424 부터는 정상 인가.

    이 함수 이전에는 코드 8 곳이 `s != "26.04.21"` 로 **0421 한 세션만** 예외 처리했다.
    26.03.24 파일에는 최대 26.5 rad/s 짜리 목표 속도가 들어 있어 그대로 먹이면 실제 로봇에
    없던 토크가 시뮬레이션에만 생긴다 (0421 에서 재 보니 감쇠x목표속도 항이 무릎 명령
    최고값의 120~232% = 명령보다 큰 유령 항). 다만 26.03.24 는 게인이 폴더에 안 적혀 있어
    PD 흉내를 아예 안 돌리므로 **지금까지의 점수에는 피해가 없었다.**
    """
    return str(sess).replace("_", ".") >= "26.04.24"
