"""P13d — 물리 구속 재적합 (사용자 실측 07-08 반영):
...
def main():
    ...
    es = cma.CMAEvolutionStrategy(((x0 - LOb) / (HIb - LOb)).tolist(), 0.08,
                                  {"bounds": [0, 1], "maxfevals": 1300, "popsize": 20,
                                   "seed": 59, "verbose": -9})
    ...
    for c in cands:
        if c["ho"] <= 1.0 and (sel is None or c["obj"] < sel["obj"]):
            sel = c
    print(f"P13d DONE nev={nev}  selected: obj={sel['obj']:.4f} ho={sel['ho']:.3f}" if sel
          else "P13d DONE: none passed ho<=1.0", flush=True)
    if sel:
        xs_ = np.array(sel["x"])
    ...
    json.dump(dict(selected=sel, names=NAMES, total_mass=TOTAL, mp_locked=MP_PHYS,
              ...
              open(REPO / "code/goal21/p13d_physical.json", "w"), indent=1)
